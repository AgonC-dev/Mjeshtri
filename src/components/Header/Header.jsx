      ...
      <div className={styles.right}>
        ...
        {user ? (
                  ...
                  {profileStrength === -1 ? (
                    <div className={styles.criticalWarning}>
                      ...
                      <p className={styles.warningText}>
                        Mungon kategoria, emri ose qyteti.
                        Klientët nuk mund t'ju gjejnë!
                      </p>
                      <Link
                        to="/dashboard"
                    ...
                    <>
                      ...
                      <div className={styles.progressBar}>
                        <div
                          ...
                          style={{
                            ...
                            backgroundColor:
                              getStrengthColor(profileStrength),
                          }}
                        />
                      </div>
                      ...
                      <p className={styles.strengthHint}>
                        {profileStrength === 100
                          ...
                          : (!userData?.profilePic || userData?.profilePic.includes('placeholder'))
                          ? "💡 Shto një foto profili për më shumë besueshmëri."
                          : (!userData?.bio || userData?.bio.trim().length <= 20)
                          ? "✍️ Shkruaj një përshkrim më të gjatë (+20 shkronja) rreth punës sate."
                          : (userData?.experienceYears === undefined || userData?.experienceYears === null || userData?.experienceYears === "")
                          ? "⏳ Shto vitet e përvojës për të treguar profesionalizmin."
                          : (userData?.startingPrice === undefined || userData?.startingPrice === null || userData?.startingPrice === "")
          ...
          <>
            ...
            <Modal open={showFavorites} onClose={() => setShowFavorites(false)}>
  <div className={styles.favModalContainer}>
    {/* Proper Close Button */}
    <button className={styles.modalCloseBtn} onClick={() => setShowFavorites(false)}>
      ✕
    </button>
      ...
      {favoritesList.length > 0 ? (
        <>
          {favoritesList.map((worker, index) => (
            ...
            >
              <img 
                src={worker.profilePic || "https://via.placeholder.com/40"} 
              ...
              <div className={styles.favMainInfo}>
                ...
                <span className={styles.favTag}>{worker.category}</span>
                <p className={styles.favLocation}>📍 {worker.city}</p>
              </div>
              
              <div className={styles.favActions}>
                <a href={`tel:${worker.phoneNum}`} className={styles.favCallBtn} title="Telefono">
                  📞 <span>Thirr</span>
                </a>
                <Link 
                  to={`/worker/${worker.id}`} 
        ...
        <div className={styles.emptyState}>
          ...
          <button className={styles.browseBtn} onClick={() => setShowFavorites(false)}>
            Shfletoni Mjeshtrit
          </button>
        </div>
      )}
...
<Modal open={sure} onClose={() => setSure(false)}>
                <div className={styles.confirmModal}>
                  ...
                  <p>A jeni i sigurt që dëshironi të fshini të gjithë listën e të preferuarve?</p>
                  <div className={styles.confirmActions}>
                    ...
                    <button className={styles.confirmNo} onClick={() => setSure(false)}>Jo, anulo</button>
                  </div>
                </div>
              </Modal>
